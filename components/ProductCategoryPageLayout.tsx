...
export default function ProductCategoryPageLayout({
  ...
  const label = collectionLabel ?? productLabel;
      ...
      <div className="grid gap-5 sm:gap-6 md:gap-8 md:grid-cols-[240px_1fr]">
        ...
        <div className="min-w-0 space-y-10 md:space-y-14">
          <SectionWrapper animate={false} className="rounded-3xl border border-[#1F3D3B]/10 bg-white/90 p-6 md:p-8">
            <Link href={baseHref} className="text-sm font-semibold text-[#F4A300] hover:text-[#ffb61f]">
            ...
            <p className="mt-3 text-sm text-[#1F3D3B]/65">
              Design inspiration from our <span className="font-semibold text-[#1F3D3B]">{category.name}</span>{" "}
              collection.
            </p>
          ...
          <SectionWrapper animate={false} className="rounded-3xl bg-[#1F3D3B] p-8 text-white md:p-10">
            <h2 className="text-2xl font-semibold md:text-3xl">Questions about {category.name} {label.toLowerCase()}?</h2>
            <p className="mt-3 max-w-2xl text-sm text-white/80 md:text-base">
              Chat with GRIHAM on WhatsApp for options, measurements, pricing, and installation. We will help you choose
              the right {label.toLowerCase()} for your space.
            </p>
            <div className="mt-6">
              <a
                href={whatsAppUrl}
