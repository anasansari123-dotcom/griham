import { curtainFilterOptions, filterHref, type CurtainFilterOption } from "@/lib/curtainsData";
import Link from "next/link";

type Props = {
  activeFilter?: CurtainFilterOption;
};

const filters = [...curtainFilterOptions];

export default function CurtainCategoryFilters({ activeFilter }: Props) {
  return (
    <aside className="h-fit rounded-2xl border border-[#1F3D3B]/10 bg-white/80 p-4 md:p-5">
      ...
      <nav className="mt-4 flex flex-wrap gap-2 md:block md:space-y-2" aria-label="Curtain style filters">
        {filters.map((filter) => (
          <Link
            key={filter}
