...
import { getSiteSettings } from "@/lib/cms/getContent";
...
export default async function ContactPage() {
  ...
  const contactItems = [
    { label: "Phone", value: settings.contactPhone, href: settings.contactPhoneHref },
    { label: "Email", value: settings.contactEmail, href: settings.contactEmailHref },
    { label: "Website", value: "www.grihamdecor.in", href: settings.contactWebsite },
    { label: "Studio Hours", value: settings.businessHours },
  ];
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-10 px-4 py-10 sm:px-6 md:space-y-14 md:py-14">
      <SectionWrapper className="rounded-3xl border border-[#1F3D3B]/10 bg-white/90 p-6 md:p-8">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#9A6800]">Contact us</p>
      ...
      <SectionWrapper className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {contactItems.map((item) => (
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#9A6800]">{item.label}</p>
            {item.href ? (
              <a
                href={item.href}
      ...
      <SectionWrapper animate={false} className="grid gap-8 md:grid-cols-2">
        <div className="rounded-3xl border border-[#1F3D3B]/10 bg-white/90 p-7 shadow-md">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#9A6800]">Visit us</p>
          ...
          <address className="mt-4 space-y-4 text-sm not-italic leading-relaxed text-[#1F3D3B]/80">
            <a href={settings.mapOpenUrl} target="_blank" rel="noreferrer" className="block transition hover:text-[#F4A300]">
              {settings.storeAddress}
            </a>
            <p>
              <span className="font-semibold text-[#1F3D3B]">Phone: </span>
              <a href={settings.contactPhoneHref} className="hover:text-[#F4A300]">
                {settings.contactPhone}
              </a>
            </p>
            <p>
              <span className="font-semibold text-[#1F3D3B]">Email: </span>
              <a href={settings.contactEmailHref} className="hover:text-[#F4A300]">
