...
import { deleteService, updateService } from "@/lib/services/cmsService";
...
export async function PUT(request: NextRequest, { params }: Props) {
  ...
  try {
    ...
    const doc = await updateService(params.id, parsed.data);
...
export async function DELETE(request: NextRequest, { params }: Props) {
  ...
  try {
    const doc = await deleteService(params.id);
    if (!doc) return jsonError("Service not found", 404);
    revalidateSite();
